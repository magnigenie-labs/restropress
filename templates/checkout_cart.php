			 ...
			 ?>
				<tr class="rpress_cart_item" id="rpress_cart_item_<?php echo esc_attr( $key ) . '_' . esc_attr( $item['id'] ); ?>" data-fooditem-id="<?php echo esc_attr( $item['id'] ); ?>">
					...
					<?php
						...
						if( isset($item['id']) ) :
							$item_price = rpress_price($item['id'], false);
						else :
							$item_price = '';
						endif;
						...
						<!-- Item Name Here -->
						<span class="rpress-cart-item-title rpress-cart-item rpress_checkout_cart_item_title"><?php echo esc_html( $item_title ); ?></span>

						<!-- Item Quantity Wrap starts Here -->
						<span class="cart-item-quantity-wrap">
						<span class="rpress_checkout_cart_item_qty">
							<?php echo $get_item_qty; ?>&nbspX&nbsp;<?php echo $item_price; ?></span>
						</span>
							...
							foreach( $cart_list_item as $k => $val ) {
								if( isset($val['addon_item_name']) && isset($val['price']) && isset($val['quantity']) ) {
									?>
										...
										<!-- Item Title -->
									 	<span class="rpress-cart-item-title"><?php echo $val['addon_item_name']; ?></span>

									 	<!-- Item Quanity Starts Here -->
									 	<span class="cart-item-quantity-wrap">
											<span class="rpress_checkout_cart_item_qty">
												<?php echo $val['quantity'].'&nbspX&nbsp;'.esc_html( rpress_currency_filter( rpress_format_amount( $val['price'] ) ) )?>
											</span>
										</span>
