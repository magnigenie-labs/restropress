...
add_shortcode( 'fooditems', 'get_rpress_fooditems' );
	...
	if( is_array($get_all_items) && !empty($get_all_items) ) :
		...
		foreach( $get_all_items as $key => $get_all_item ) {
			...
			while ( $fooditems->have_posts() ) : $fooditems->the_post();
				$post_id = get_the_ID();
				$post_terms = wp_get_post_terms($post_id, 'food-category');

				$current_food_cat = @$get_food_cat;
				$get_food_cat = $post_terms[0]->name;
				$get_food_id = $post_terms[0]->term_taxonomy_id;
				$get_description = $post_terms[0]->description;	
