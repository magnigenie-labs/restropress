 ...
 *
 * Generates PDF report on sales and earnings for all fooditems for the current year.
 *
 * @since 1.1.4.0
 * @param string $data
 * @uses rpress_pdf
...
function rpress_generate_pdf( $data ) {
	...
	if ( $fooditems ):
		...
		foreach ( $fooditems as $fooditem ):
			...
			$title = utf8_decode( get_the_title( $fooditem->ID ) );

			if ( rpress_has_variable_prices( $fooditem->ID ) ) {

				$prices = rpress_get_variable_prices( $fooditem->ID );
				...
				$price = html_entity_decode( rpress_currency_filter( rpress_format_amount( $min ) ) . ' - ' . rpress_currency_filter( rpress_format_amount( $max ) ) );
			} else {
				$price = html_entity_decode( rpress_currency_filter( rpress_get_fooditem_price( $fooditem->ID ) ) );
			}

			$categories = get_the_term_list( $fooditem->ID, 'addon_category', '', ', ', '' );
			$categories = $categories ? strip_tags( $categories ) : '';

			$tags = get_the_term_list( $fooditem->ID, 'fooditem_tag', '', ', ', '' );
			$tags = $tags ? strip_tags( $tags ) : '';

			$sales = rpress_get_fooditem_sales_stats( $fooditem->ID );
			$earnings = html_entity_decode ( rpress_currency_filter( rpress_get_fooditem_earnings_stats( $fooditem->ID ) ) );
	...
	$pdf->Ln( 7 );
	$pdf->Output( apply_filters( 'rpress_sales_earnings_pdf_export_filename', 'rpress-report-' . date_i18n('Y-m-d') ) . '.pdf', 'D' );
}
add_action( 'rpress_generate_pdf', 'rpress_generate_pdf' );
...
function rpress_draw_chart_image() {
	...
	while ( $i <= 12 ) :
		$earnings .= rpress_get_earnings_by_date( null, $i, date('Y') ) . ",";
		$sales .= rpress_get_sales_by_date( null, $i, date('Y') ) . ",";
		$i++;
	endwhile;
