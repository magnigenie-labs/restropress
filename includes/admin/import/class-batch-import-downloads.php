...
/**
 * RPRESS_Batch_RestroPress_Import Class
 *
 * @since 1.0.0
 */
class RPRESS_Batch_RestroPress_Import extends RPRESS_Batch_Import {
		...
		if( ! $this->done && $this->csv->data ) {
			...
			foreach( $this->csv->data as $key => $row ) {
				...
				if( ! empty( $this->field_mapping['categories'] ) && ! empty( $row[ $this->field_mapping['categories'] ] ) ) {
					...
					$this->set_taxonomy_terms( $fooditem_id, $categories, 'addon_category' );
				...
				if( ! empty( $this->field_mapping['tags'] ) && ! empty( $row[ $this->field_mapping['tags'] ] ) ) {
					...
					$this->set_taxonomy_terms( $fooditem_id, $tags, 'fooditem_tag' );
				...
				if( ! empty( $this->field_mapping['files'] ) && ! empty( $row[ $this->field_mapping['files'] ] ) ) {
					...
					$this->set_files( $fooditem_id, $files );
				...
				// File fooditem limit
				if( ! empty( $this->field_mapping['fooditem_limit'] ) && ! empty( $row[ $this->field_mapping['fooditem_limit'] ] ) ) {

					update_post_meta( $fooditem_id, '_rpress_fooditem_limit', absint( $row[ $this->field_mapping['fooditem_limit'] ] ) );
				}
	 ...
	 * @return void
	 */
	private function set_taxonomy_terms( $fooditem_id = 0, $terms = array(), $taxonomy = 'addon_category' ) {
	 ...
	 * @return array
	 */
	private function maybe_create_terms( $terms = array(), $taxonomy = 'addon_category' ) {
