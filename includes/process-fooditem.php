 ...
 * Process FoodItem
 *
 * Handles the file fooditem process.
 *
 * @access      private
 * @since       1.0
 * @return      void
 */
function rpress_process_fooditem() {

	if( ! isset( $_GET['fooditem_id'] ) && isset( $_GET['fooditem'] ) ) {
		$_GET['fooditem_id'] = $_GET['fooditem'];
	}

	$args = apply_filters( 'rpress_process_fooditem_args', array(
		'fooditem' => ( isset( $_GET['fooditem_id'] ) )  ? (int) $_GET['fooditem_id']                       : '',
		...
		'key'      => ( isset( $_GET['fooditem_key'] ) ) ? $_GET['fooditem_key']                            : '',
		'rpressfile'  => ( isset( $_GET['rpressfile'] ) )      ? $_GET['rpressfile']                                 : '',
		'ttl'      => ( isset( $_GET['ttl'] ) )          ? $_GET['ttl']                                     : '',
		'token'    => ( isset( $_GET['token'] ) )        ? $_GET['token']                                   : ''
	) );

	if ( ! empty( $args['rpressfile'] ) && ! empty( $args['ttl'] ) && ! empty( $args['token'] ) ) {

		// Validate a signed URL that rpress_process_signed_fooditem_urlcontains a token
		$args = rpress_process_signed_fooditem_url( $args );

		// Backfill some legacy super globals for backwards compatibility
		$_GET['fooditem_id']  = $args['fooditem'];
		$_GET['email']        = $args['email'];
		$_GET['expire']       = $args['expire'];
		$_GET['fooditem_key'] = $args['key'];
		$_GET['price_id']     = $args['price_id'];

	} elseif ( ! empty( $args['fooditem'] ) && ! empty( $args['key'] ) && ! empty( $args['email'] ) && ! empty( $args['expire'] ) && isset( $args['file_key'] ) ) {

		// Validate a legacy URL without a token
		$args = rpress_process_legacy_fooditem_url( $args );
	...
	$args['has_access'] = apply_filters( 'rpress_file_fooditem_has_access', $args['has_access'], $args['payment'], $args );

	//$args['has_access'] = ( rpress_logged_in_only() && is_user_logged_in() ) || !rpress_logged_in_only() ? true : false;
	if ( $args['payment'] && $args['has_access'] ) {

		do_action( 'rpress_process_verified_fooditem', $args['fooditem'], $args['email'], $args['payment'], $args );

		// Determine the fooditem method set in settings
		$method  = rpress_get_file_fooditem_method();

		// Payment has been verified, setup the fooditem
		$fooditem_files = rpress_get_fooditem_files( $args['fooditem'] );
		$attachment_id  = ! empty( $fooditem_files[ $args['file_key'] ]['attachment_id'] ) ? absint( $fooditem_files[ $args['file_key'] ]['attachment_id'] ) : false;
		$thumbnail_size = ! empty( $fooditem_files[ $args['file_key'] ]['thumbnail_size'] ) ? sanitize_text_field( $fooditem_files[ $args['file_key'] ]['thumbnail_size'] ) : false;
		$requested_file = isset( $fooditem_files[ $args['file_key'] ]['file'] ) ? $fooditem_files[ $args['file_key'] ]['file'] : '';
		 ...
		 */
		if( rpress_is_local_file( $requested_file ) && $attachment_id && 'attachment' == get_post_type( $attachment_id ) ) {

			if( 'pdf' === strtolower( rpress_get_file_extension( $requested_file ) ) ) {
				// Do not ever grab the thumbnail for PDFs. See 
		...
		$requested_file = apply_filters( 'rpress_requested_file', $requested_file, $fooditem_files, $args['file_key'] );
		...
		if ( ( ! isset( $file_details['scheme'] ) || ! in_array( $file_details['scheme'], $schemes ) ) && isset( $file_details['path'] ) && file_exists( $requested_file ) ) {
			...
			/**
			 * FoodItem method is set to Redirect in settings but an absolute path was provided
			 * We need to switch to a direct fooditem in order for the file to fooditem properly
			 */
			$method = 'direct';
		...
		/**
		 * Allow extensions to run actions prior to recording the file fooditem log entry
		 *
		 * @since 1.0.0.14
		 */
		do_action( 'rpress_process_fooditem_pre_record_log', $requested_file, $args, $method );

		// Record this file fooditem in the log
		$user_info = array();
		...
		rpress_record_fooditem_in_log( $args['fooditem'], $args['file_key'], $user_info, rpress_get_ip(), $args['payment'], $args['price_id'] );

		$file_extension = rpress_get_file_extension( $requested_file );
		$ctype          = rpress_get_file_ctype( $file_extension );
		...
		do_action( 'rpress_process_fooditem_headers', $requested_file, $args['fooditem'], $args['email'], $args['payment'] );

		nocache_headers();
		header("Robots: none");
		header("Content-Type: " . $ctype . "");
		header("Content-Description: File Transfer");
		header("Content-Disposition: attachment; filename=\"" . apply_filters( 'rpress_requested_file_name', basename( $requested_file ), $args ) . "\"");
		header("Content-Transfer-Encoding: binary");

		// If the file isn't locally hosted, process the redirect
		if ( filter_var( $requested_file, FILTER_VALIDATE_URL ) && ! rpress_is_local_file( $requested_file ) ) {
			rpress_deliver_fooditem( $requested_file, true );
			exit;
		}
		...
		switch( $method ) :
			...
			default:
				...
				if ( stristr( getenv( 'SERVER_SOFTWARE' ), 'lighttpd' ) ) {
					...
					$ignore_x_accel_redirect_header = apply_filters( 'rpress_ignore_x_accel_redirect', false );
				...
				if( $direct ) {
					...
					rpress_deliver_fooditem( $file_path );
...
add_action( 'init', 'rpress_process_fooditem', 100 );

/**
 * Deliver the fooditem file
 *
 * If enabled, the file is symlinked to better support large file fooditems
 *
 * @param    string    $file
 * @param    bool      $redirect True if we should perform a header redirect instead of calling rpress_readfile_chunked()
 * @return   void
 */
function rpress_deliver_fooditem( $file = '', $redirect = false ) {
	 ...
	 */
	if( rpress_symlink_file_fooditems() && rpress_is_local_file( $file ) ) {

		$file = rpress_get_local_path_from_url( $file );

		// Generate a symbolic link
		$ext       = rpress_get_file_extension( $file );
		$parts     = explode( '.', $file );
		$name      = basename( $parts[0] );
		$md5       = md5( $file );
		$file_name = $name . '_' . substr( $md5, 0, -15 ) . '.' . $ext;
		$path      = rpress_get_symlink_dir() . '/' . $file_name;
		$url       = rpress_get_symlink_url() . '/' . $file_name;
		...
		if ( ! wp_next_scheduled( 'rpress_cleanup_file_symlinks' ) ) {
			wp_schedule_single_event( current_time( 'timestamp' ) + 60, 'rpress_cleanup_file_symlinks' );
		}
 ...
 */
function rpress_is_local_file( $requested_file ) {
 ...
 */
function rpress_get_local_path_from_url( $url ) {

	$file       = $url;
	$upload_dir = wp_upload_dir();
	$upload_url = $upload_dir['baseurl'] . '/rpress';
	...
	if( defined( 'UPLOADS' ) && strpos( $file, UPLOADS ) !== false ) {
		...
		/** This is a local file given by URL so we need to figure out the path */
		$file = str_replace( $upload_url, rpress_get_upload_dir(), $file );

	} else if( strpos( $file, set_url_scheme( $upload_url, 'https' ) ) !== false ) {

		/** This is a local file given by an HTTPS URL so we need to figure out the path */
		$file = str_replace( set_url_scheme( $upload_url, 'https' ), rpress_get_upload_dir(), $file );
 ...
 * @return   string
 */
function rpress_get_file_ctype( $extension ) {
	switch( $extension ):
		...
		default         : $ctype = "application/force-fooditem";
	endswitch;
	...
	return apply_filters( 'rpress_file_ctype', $ctype );
}

/**
 * Reads file in chunks so big fooditems are possible without changing PHP.INI
 ...
 */
function rpress_readfile_chunked( $file, $retbytes = true ) {
	...
	// If output buffers exist, make sure they are closed. See 
	if ( ob_get_length() ) {
		ob_clean();
	}
	...
	// Only send partial content header if fooditeming a piece of the file (IE workaround)
...
/**
 * Used to process an old URL format for fooditems
 *
 * @since  1.0.0
 * @param  array $args Arguments provided to fooditem a file
 * @return array       Same arguments, with the status of verification added
 */
function rpress_process_legacy_fooditem_url( $args ) {

	// Verify the payment
	$args['payment'] = rpress_verify_fooditem_link( $args['fooditem'], $args['key'], $args['email'], $args['expire'], $args['file_key'] );
...
/**
 * Used to process a signed URL for processing fooditems
 *
 * @since  1.0.0
 * @param  array $args Arguments provided to fooditem a file
 * @return array       Same arguments, with the status of verification added
 */
function rpress_process_signed_fooditem_url( $args ) {

	$parts = parse_url( add_query_arg( array() ) );
	wp_parse_str( $parts['query'], $query_args );
	$url = add_query_arg( $query_args, site_url() );

	$valid_token = rpress_validate_url_token( $url );

	// Bail if the token isn't valid.
	// The request should pass through RPRESS, or custom handling can be enabled with the action.
	if ( ! $valid_token ) {
		$args['payment']    = false;
	...
	$order_parts = explode( ':', rawurldecode( $_GET['rpressfile'] ) );
	...
	$args['fooditem']    = $order_parts[1];
	$args['payment']     = $order_parts[0];
	$args['file_key']    = $order_parts[2];
	$args['price_id']    = $order_parts[3];
	$args['email']       = rpress_get_payment_meta( $order_parts[0], '_rpress_payment_user_email', true );
	$args['key']         = rpress_get_payment_meta( $order_parts[0], '_rpress_payment_purchase_key', true );

	$payment = new RPRESS_Payment( $args['payment'] );
...
/**
 * Determines if we should use symbolic links during the file fooditem process
 *
 * @since  1.0.0
 * @return bool
 */
function rpress_symlink_file_fooditems() {
	$symlink = rpress_get_option( 'symlink_file_fooditems', false ) && function_exists( 'symlink' );
	return (bool) apply_filters( 'rpress_symlink_file_fooditems', $symlink );
}
 ...
 * @param  array  $fooditem_files The fooditem files
 * @param  string $file_key       The file key
 * @return string                 The file (if local) with the matched scheme
 */
function rpress_set_requested_file_scheme( $requested_file, $fooditem_files, $file_key ) {
...
add_filter( 'rpress_requested_file', 'rpress_set_requested_file_scheme', 10, 3 );

/**
 * Perform a head request on file URLs before attempting to fooditem to check if they are accessible.
 *
 * @since 1.0.0
 * @param  string $requested_file The Requested File
 * @param  array  $args           Arguments
 * @param  string $method         The fooditem mehtod being sed
 * @return void
 */
function rpress_check_file_url_head( $requested_file, $args, $method ) {
	...
	if( filter_var( $requested_file, FILTER_VALIDATE_URL ) && ! rpress_is_local_file( $requested_file ) ) {
		...
		if( ! $valid ) {
			...
			do_action( 'rpress_check_file_url_head_invalid', $requested_file, $args, $method );
...
/**
 * Filter removed in RPRESS 2.7
 *
 */
// add_action( 'rpress_process_fooditem_pre_record_log', 'rpress_check_file_url_head', 10, 3 );
